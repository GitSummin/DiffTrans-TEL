...
"""
Stage 3 — Inpainting via MAT (Mask-Aware Transformer)

Takes the composited images and boundary masks from Stage 2b and runs MAT
inference to seamlessly blend the pasted TEL objects into the background.

MAT (Li et al., 2022) is a StyleGAN2-based inpainting model that uses a
Transformer in the latent space to model long-range dependencies, enabling
photorealistic filling of the seam region around each inserted object.

Inputs:
  --dpath   Directory of composited RGB images (Stage 2b output: images/)
  --mpath   Directory of corresponding inpainting masks (Stage 2b output: masks/)
  --network Pretrained MAT model pickle (.pkl)
  --outdir  Directory for final blended output images

Output:
  One inpainted PNG per input image, saved to --outdir.
  These images can be used directly as training data for a downstream
  aerial vehicle detector (e.g. YOLOv8).

Usage:
  python generate_image.py \
      --network  pretrained/Places_512_FullData_G.pkl \
      --dpath    composites/images/ \
      --mpath    composites/masks/ \
...
import os
os.environ['TORCH_CUDA_ARCH_LIST'] = '8.6'  # e.g. '7.5' for Turing GPUs, '8.0' for A100
...
def generate_images(
    ...
    with torch.no_grad():
        for i, ipath in enumerate(img_list):
            ...
            print(f'→ Elapsed time for {iname}: {elapsed_time:.2f} seconds')
