"""
MAT Generator and Discriminator (Stage 3 core network)

Architecture overview (Generator):
  1. Encoder (ConvolutionEncoder):
       Progressive Conv2dLayerPartial blocks with partial convolutions that
       ignore masked pixels, producing multi-scale feature maps.
  2. Middle Transformer (StyleSwinTransformerBlock stack):
       Window-based self-attention (Swin-style) on the latent feature map,
       conditioned on StyleGAN2 style codes from the mapping network.
       Captures long-range context across the masked region.
  3. Decoder (upsampling blocks):
       Progressive StyleConv + ToRGB blocks with skip connections from the
       encoder, producing the inpainted output at the original resolution.
  4. Mapping Network:
       Maps a random noise vector z to W-space style codes that modulate
       each synthesis layer (StyleGAN2 design).

  The key innovation is the style-modulated Transformer in the bottleneck:
  it lets the model reason about global image structure while keeping the
  generation diversity of a GAN.

Reference:
  Li et al., "MAT: Mask-Aware Transformer for Large Hole Image Inpainting",
  CVPR 2022.  https://arxiv.org/abs/2203.15270
"""
...
class FirstStage(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        # ensemble
        img = img * (1 - masks_in) * 0.8 + images_in * masks_in * 0.2
