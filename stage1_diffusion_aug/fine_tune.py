"""
Stage 1 — Textual Inversion Fine-Tuning for TEL Classes

Fine-tunes a new token embedding for each TEL vehicle class using a small set
of real reference images (as few as 3-5 images per class).

Process:
  1. Register a placeholder token (e.g. <buk_m2>) in the CLIP tokenizer.
  2. Freeze the entire SD model except the token embedding being learned.
  3. Optimize the embedding to minimize the diffusion reconstruction loss on
     the real reference images paired with the text prompt
     "a photo of a <buk_m2>".
  4. Save the learned embedding to learned_embeds.bin for use in
     generate_images.py.
...
from semantic_aug.datasets.tel import TelDataset
