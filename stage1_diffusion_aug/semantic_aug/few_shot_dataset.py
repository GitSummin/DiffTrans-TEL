...
class FewShotDataset(Dataset):
    ...
    def generate_augmentations(self, num_repeats: int):
        ...
        print("Augumentation class : ",self.class_names)
        for idx, num in tqdm(list(
            ...
            synthetic_name = self.all_images[idx].split("\\")[-1].split(".")[0]
            if self.synthetic_dir is not None:
                pil_image, image = image, os.path.join(
                    self.synthetic_dir, f"aug-{synthetic_name}-{num}.png")
                pil_image.save(image)
            self.synthetic_examples[idx].append((image, label))
            
            self.input_list.append(self.all_images[idx])
            self.output_list.append(image)
