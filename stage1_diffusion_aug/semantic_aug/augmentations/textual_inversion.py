"""
Textual Inversion Augmentation (Stage 1 core module)

Wraps a Stable Diffusion Img2Img or Inpainting pipeline with a class-specific
learned token embedding to perform appearance-preserving augmentation.

Key idea:
  After fine-tuning (fine_tune.py), each TEL class is represented by a unique
  token (e.g. <buk_m2>) whose embedding captures its visual appearance.
  At inference, the token is inserted into a text prompt and SD is conditioned
  on both the prompt and a source image (img2img) or a region mask (inpaint),
  producing a diverse set of on-white-background TEL images.
"""
...
def load_embeddings(embed_path: str,
    ...
    """Load a Textual Inversion embedding file into the CLIP tokenizer/encoder.

    The .bin file (produced by fine_tune.py) maps each custom token string to
    its learned embedding vector.  We expand the tokenizer vocabulary and patch
    the weight matrix of the text encoder so that downstream SD inference can
    use the custom token as a normal text prompt word.
    """
...
class TextualInversion(GenerativeAugmentation):
    """SD-based augmentation using a class-specific Textual Inversion token.

    Supports two modes controlled by the `mask` flag:
      - img2img (mask=False): the whole image is re-rendered with the custom token.
      - inpaint (mask=True):  only the masked region (foreground object) is
        re-generated, preserving the background — used for data augmentation
        where only the object appearance should vary.
    """

    pipe = None  # shared across instances to avoid reloading the model into GPU memory
